...
// Lista para mantener el registro de usuarios
const usersInRoom = {};
...
io.on('connection', socket => {
  socket.on('join-room', (roomId, userId, userName) => {
    ...
    // Le enviamos la lista de usuarios ya conectados al nuevo usuario
    socket.emit('all-users', usersInRoom[roomId]);

    // Agregamos el nuevo usuario a la lista y emitimos el evento de que se ha unido
...
// Sirve el archivo HTML que contiene la aplicación React
app.get('/', (req, res) => {
  ...
  res.send(`
    ...
    <body class="bg-gray-900">
      <div id="root" class="flex items-center justify-center h-screen text-white text-lg">
        Cargando...
      </div>
      <script type="text/babel">
        // Se utiliza un intervalo para verificar la carga de ReactDOM y lucideReact.
        const checkLibraries = setInterval(() => {
          if (typeof ReactDOM !== 'undefined' && typeof window.lucideReact !== 'undefined') {
